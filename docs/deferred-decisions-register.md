...
| Permission scoping | **Resolved (2026-09-16)** — implemented exactly as the domain doc's own suggested default (ADR-TR04): `TrainingProgram` `ADMIN`-only, `Enrollment` splits `create:own`/`:any`, `read:own`/`:any`, `manage:any`, `withdraw:own`, with no `MANAGER` reports-visibility (no textual basis for one, unlike Leave/Performance). |
...
| Permission scoping | **Resolved (2026-09-22)** — flat `ADMIN`-only for `Asset` and assign/return (no IT role exists; no aggregate has a natural "own" concept for mutations), with a `assetAssignment:read:own`/`:read:any` split so employees can see their own held assets (ADR-AM05). |
...
| Payroll final partial-month Payslip for a separated employee | Surfaced by Exit Management's implementation (2026-09-22): `processPayrollRun` excludes soft-deleted employees and there is no per-employee generation. Needs an additive Payroll change; Exit Management does not write into Payroll. |
| Wiring a scheduler to `POST /exit-cases/process-due` | No scheduler/cron infrastructure exists project-wide; the sweep is an explicit endpoint until one does (ADR-EM07). |
| Permission scoping | **Resolved (2026-09-22)** — `ADMIN`-only for initiating terminations and managing cases (no HR role exists), `create:own`/`read:own`/`withdraw:own` for employees, no `MANAGER` reports-visibility (ADR-EM06). |
...
|---|---|
| Permission scoping beyond `ADMIN`-only | Branch (ADR-B07), Department (ADR-D08), Designation (ADR-DS06), Holiday Calendar (ADR-HC06), Shift (ADR-SH05), LeaveType (ADR-LV07), PayrollRun (ADR-PR06), ReviewCycle (ADR-PF05), Recruitment - `JobRequisition`/`Candidate`/`Application` (ADR-RC05), Training's `TrainingProgram` (ADR-TR04), and Asset Management's `Asset` (ADR-AM05) have all resolved this as `ADMIN`-only mutations / read-for-all (2026-09-13 through 2026-09-22) — the same resolution is the likely default for the rest unless a real requirement diverges. Attendance (ADR-AT06), LeaveRequest/LeaveBalance (ADR-LV07), Payslip (ADR-PR06), PerformanceReview (ADR-PF05), Training's `Enrollment` (ADR-TR04), Asset Management's `AssetAssignment` (ADR-AM05, mutations `ADMIN`-only but reads split `:own`/`:any`), and now Exit Management's `ExitCase` (ADR-EM06, `ADMIN`-only for termination/manage, employees `create:own`/`read:own`/`withdraw:own`) instead resolved to an own/any split, since all five are self-service-plus-admin-correction (or workflow-with-authoring-authority) domains, not pure master data - a second, equally-established pattern now exists depending on domain shape. Payslip's own/any split further diverges from Leave's by withholding `MANAGER`'s reports-visibility, since no verified requirement extends pay visibility to managers; PerformanceReview instead extends Leave's manager-plus-admin-fallback shape from a single `decide:*` action to two separate authority axes (`create:*` for authoring, `manage:*` for lifecycle transitions). Recruitment's resolution notably has *no* own/any axis at all (unlike every self-service-shaped domain) - no Recruitment aggregate has a natural "own" concept, since Candidate isn't even a `User`. Enrollment's own/any split diverges from Leave's/Performance's in the opposite direction from Payslip: no `MANAGER` reports-visibility at all (not even a narrowed one) - the domain doc's own "who performs" text never mentions managers, only `ADMIN`/HR and self-enrollment. |
...
| No notification infrastructure project-wide | Training (renewal reminders) is the only domain to name this explicitly, but it would also affect Leave (approval notifications) and Exit Management (clearance reminders) once built. Still true as of Training's implementation (2026-09-16) - compliance tracking remains passive/query-only, per ADR-TR02/§7's own accepted trade-off. |
| No scheduler/cron infrastructure project-wide | Leave (accrual jobs, noted in `leave.service.js`) and Exit Management (time-based separation, realized as an explicit `POST /exit-cases/process-due` sweep, ADR-EM07) both need time-triggered work with nothing to run it. |
| AuditLog extension to new entity types | Branch (ADR-B08), Department (ADR-D09), Designation, Holiday Calendar (both entities, `HolidayCalendar` and `Holiday`), Shift, Attendance (ADR-AT04), Leave (all three entities - `LeaveType`, `LeaveRequest`, `LeaveBalance`), Payroll (both `PayrollRun` and `Payslip`), Performance (all three entities - `ReviewCycle`, `PerformanceReview`, `ReviewAddendum`), Recruitment (all six entities - `JobRequisition`, `Candidate`, `CandidateDocument`, `Application`, `Interview`, `Offer`), Training (all three entities - `TrainingProgram`, `Enrollment`, `EnrollmentDocument`), Asset Management (both entities - `Asset`, `AssetAssignment`), and now Exit Management (both entities - `ExitCase`, `ClearanceItem`) have all resolved this (2026-09-13 through 2026-09-22) — confirms the generic `AuditLog` model extends cleanly with no schema change, as predicted, across fourteen independent domains now. |
